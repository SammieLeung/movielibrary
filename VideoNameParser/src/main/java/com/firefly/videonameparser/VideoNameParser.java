...
//			if (nameParts.size() == 1 && !isNaN(nameParts.get(0))) break; /* Only a number: unacceptable */ 
	 ...
	 * */
	private final static String MATCH_YEAR_REGEX = "[^\\d](19[0-9][0-9]|20[0-1][0-9])[^\\d]";//匹配４位数字,范围为1900-2099
	...
	private void parserEpisode(String seg)
	{
		...
		for (String split : splits) {
			...
			String episode_regex=build_or_pattern(EPISODE_WORDS)+"(\\d{1,2})";//"E(\\d{1,2})"
