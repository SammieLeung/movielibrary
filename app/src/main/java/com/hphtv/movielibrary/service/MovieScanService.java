...
public class MovieScanService extends Service {
    ...
    public void scanVideo(Shortcut shortcut, List<VideoFile> videoFileList) {
        startSearch2(shortcut, videoFileList, shortcut.folderType);
    }
    ...
    AtomicInteger mGlobalTaskCount = new AtomicInteger();//
     ...
     * @param searchType
     */
    private void startSearch2(Shortcut shortcut, List<VideoFile> videoFileList, Constants.SearchType searchType) {
        Log.e(TAG, "startSearch2: " + searchType.name() + "模式");
        //combineLatest 将前面Observable的最新数据与最后的Observable发送的每个数据结合
        Object lock = new Object();
        AtomicInteger mSuccessCount = new AtomicInteger();
        AtomicInteger mIndexCount=new AtomicInteger();
        AtomicInteger mCurrentTaskCount = new AtomicInteger();
                ...
                new Function3<Shortcut, Constants.SearchType, List<VideoFile>, Object[]>() {
                    ...
                    public Object[] apply(Shortcut shortcut1, Constants.SearchType searchType1, List<VideoFile> videoFileList) throws Throwable {
                        ...
                        Observable.fromIterable(videoFileList)
                                ...
                                .map(new Function<VideoFile, Object[]>() {
                                    @Override
                                    public Object[] apply(VideoFile videoFile) throws Throwable {
...
//                                })
                                .subscribe(new SimpleObserver<Object[]>() {
                                    ...
                                    public void onSubscribe(Disposable d) {
                                        ...
                                        if (mCurrentTaskCount.getAndIncrement() == 0) ;
                                        {
                                            Intent intent = new Intent();
                                            intent.setAction(Constants.BroadCastMsg.SHORTCUT_SCRAP_START);
                                            intent.putExtra(Constants.Extras.SHORTCUT_URI, shortcut1.uri);
                                            LocalBroadcastManager.getInstance(MovieScanService.this).sendBroadcast(intent);
                                        }
                                    ...
                                    public void onAction(Object[] data) {
                                        int scannedCount=mIndexCount.incrementAndGet();
                                        if (data.length == 2) {
                                            String movieId = (String) data[0];
                                            Shortcut st = (Shortcut) data[1];
                                            if (!movieId.equals("-1")) {
                                                int success = mSuccessCount.incrementAndGet();
                                    ...
                                    public void onComplete() {
                                        ...
                                        synchronized (lock) {
                                            if (mCurrentTaskCount.decrementAndGet() == 0) {
                                                //文件夹扫描结束
                                                Intent intent = new Intent();
    ...
    private void sendMatchMovieSuccess(Shortcut shortcut,String movie_id,int success,int scanned_count,int total){
        ...
        intent.putExtra(Constants.Extras.SHORTCUT_URI, shortcut.uri);
        LocalBroadcastManager.getInstance(MovieScanService.this).sendBroadcast(intent);
    }
